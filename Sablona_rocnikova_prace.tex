...
\subsubsection{Vývoj a editace}
Program byl vyvíjen v nástroji \textit{Code Blocks}, v repositáři se nachází nastavený projekt. Editace vstupu probíhá v přímo v kódu, v souboru \textit{main.c}. Rozhodl jsem se zvolit tento ne až tak praktický způsob, jelikož jsem chtěl primárně ukázat rozdíl mezi imperativním a neimperativním způsobem programování. 
\subsubsection{Princip fungování}
Program vezme vstup, který mu zadal uživatel (modifikoval pole \textit{vstupniData}), převede ho do pole datového typu \textit{prefix}. Datový typ prefix obsahuje dvě proměnné z nich první je znak a druhá číslo. Pokud je zadáno číslo a 
